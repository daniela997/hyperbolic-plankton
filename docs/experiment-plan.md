...
| **E0** | `--geometry euclidean` (two sub-runs E0a/E0b, see below) | the missing corner of the 2×2: LoRA in **flat** space (cosine CLIP InfoNCE = open_clip `ClipLoss`, no SEL). Isolates LoRA-vs-full-FT from hyperbolic-vs-Euclidean | E0b (B0-matched) vs B0 = pure geometry delta; E0a (HAC LoRA recipe) vs Planktonzilla = LoRA-vs-full-FT. CL-only, `--lambda-sel` forced 0. Launch: `scripts/run_ablations_euclidean.sh` |
