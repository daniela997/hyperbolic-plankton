...
## Setup: download BIOSCAN + the exact split (do this first on a new machine)

The seen/unseen split is **not generated by us** — it ships inside CLIBD's pre-split HDF5
file as named groups (`train_seen`, `test_seen`, `test_unseen`, `val_seen`, `val_unseen`).
So "reproduce the exact split" = download the canonical file; there is no split script to run.
It is the same split used by CLIBD and the Hyperbolic-Taxonomies (2025) paper.

1. **Download** the CLIBD split data from HuggingFace (`bioscan-ml/clibd`). The HDF5 ships as
   multi-part files that must be concatenated:
   ```bash
   pip install huggingface_hub
   huggingface-cli download bioscan-ml/clibd --repo-type dataset \
     --include "split_data/*" --local-dir /your/path/clibd
   # merge the 2 split parts (part001, part002) into the single ~80GB HDF5 the loader expects:
   cat /your/path/clibd/split_data/splitted_files/BioScan_data_in_splits.hdf5.part001 \
       /your/path/clibd/split_data/splitted_files/BioScan_data_in_splits.hdf5.part002 \
     > /your/path/BioScan_data_in_splits.hdf5
   ```
   The merged file is ~80 GB; ensure the target disk has room. (If CLIBD changes the part
   naming, `cat .../BioScan_data_in_splits.hdf5.part* > merged.hdf5` in sorted order works.)

2. **Point the code at it** via the `BIOSCAN_HDF5` env var (the scripts read it, falling back
   to the original `/scratch/daniela/...` path if unset):
   ```bash
   export BIOSCAN_HDF5=/your/path/BioScan_data_in_splits.hdf5
   ```

3. **Verify** the file has the expected groups before training:
   ```bash
   python -c "import h5py; f=h5py.File('$BIOSCAN_HDF5'); print([k for k in f if 'train' in k or 'test' in k])"
   # expect to include: train_seen (36,279 imgs), test_seen (4,878), test_unseen (6,448)
   ```

No download/merge is needed for Planktonzilla here — this baseline is BIOSCAN-only. The
BIOSCAN images decode from the HDF5 directly (no separate image dir).
